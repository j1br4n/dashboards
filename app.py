from flask import Flask, jsonify, render_template, Response, abort, make_response, request
import sqlite3
import pathlib
import logging
import requests
...
def get_customer_retention_data():  # per month
    return [
        {"date": "2023-05-01", "retention_rate": 0.75},
        {"date": "2023-06-01", "retention_rate": 0.81},
        {"date": "2023-07-01", "retention_rate": 0.78},
        {"date": "2023-08-01", "retention_rate": 0.76},

    ]

def get_product_sales_by_region():
    return [
        {"region": "North America", "sales": 1200},
        {"region": "Europe", "sales": 800},
        {"region": "Asia", "sales": 950},
    ]

def get_shipping_time_over_time(): # in hours per month
    return [
        {"date": "2023-05-01", "shipping_time": 3},
        {"date": "2023-06-01", "shipping_time": 2.5},
        {"date": "2023-07-01", "shipping_time": 4.1},
        {"date": "2023-08-01", "shipping_time": 4.7},

    ]

@app.route("/api/customer_retention")
def customer_retention():
    data = get_customer_retention_data()
    return jsonify(data)

@app.route("/api/product_sales_by_region")
def product_sales_by_region():
    data = get_product_sales_by_region()
    return jsonify(data)

@app.route("/api/shipping_time_over_time")
def shipping_time_over_time():
    data = get_shipping_time_over_time()
    return jsonify(data)
...
@app.route("/api/orders_over_time_filtered", methods=["GET"])
def orders_over_time_filtered() -> Response:
    ...
    try:
        ...
        logging.error("Error in /api/orders_over_time_filtered: %s", e)
        abort(500, description="Error processing filtered data.")
